...
echo "build image(maven-tomcat)"
cd build/maven && docker build -t ${imageReg}/hawk_maven_tomcat_build:${version} -f tomcat/Dockerfile .
docker push ${imageReg}/hawk_tomcat_build:${version}
cd ${root}
#
echo "build image(maven-springboot)"
cd build/maven && docker build -t ${imageReg}/hawk_maven_springboot_build:${version} -f springboot/Dockerfile .
docker push ${imageReg}/hawk_springboot_build:${version}
cd ${root}
#
